...
export const Menu = ({ setActive, children }: { setActive: (item: string | null) => void; children: React.ReactNode }) => {
	return (
		<nav
			...
			className='relative rounded-full border border-transparent bg-black-100 bg-opacity-40 backdrop-blur-md dark:border-white/[0.2] shadow-input flex justify-center space-x-4 px-8 py-6 '>
			{children}
		</nav>
	);
};
