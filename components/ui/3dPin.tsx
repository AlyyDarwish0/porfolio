...
export const PinContainer = ({ children, title, href, className, containerClassName }: { children: React.ReactNode; title?: string; href: string; className?: string; containerClassName?: string }) => {
	...
	return (
		<div className={cn("relative group/pin cursor-pointer", containerClassName)} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} onClick={() => (window.location.href = href)}>
			<div
				style={{
					perspective: "1000px",
				...
				<div
					...
					className='absolute left-1/2 top-1/2  flex justify-start items-start  rounded-2xl group-hover/pin:border-white/[0.2] transition duration-700 overflow-hidden'>
